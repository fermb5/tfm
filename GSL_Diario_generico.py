...
for k in range(indices_rellenar.shape[0]):
    indices_anual=pd.DataFrame({'Año':['TMY'],
                'DiasGSL':[indices_rellenar.iloc[k,1]-indices_rellenar.iloc[k,0]+1],
                'MesIni': [temp.iloc[indices_rellenar.iloc[k,0],0]],
                'DiaIni': [temp.iloc[indices_rellenar.iloc[k,0],1]],
                'MesFin': [temp.iloc[indices_rellenar.iloc[k,1],0]],
                'DiaFin': [temp.iloc[indices_rellenar.iloc[k,1],1]]})
...
for k in range(indices_rellenar.shape[0]):
    ...
    if indices_anual.iloc[0,2]>=6:            
        tempanual=pd.concat([tempanual,indices_anual],ignore_index=True) 
GSLAnual=pd.concat([GSLAnual,tempanual],ignore_index=True)
filename='PSA_GSL_movmedian10_TMY.xlsx'
