...
for i in range(len(Anio)):
    ...
    os.chdir(ruta)
    filenameMens='PSAyMet_'+Anio[i]+'_EOT_Hor.xlsx'
    ClimaTrat=pd.read_excel(filenameMens)
    
    ruta2='D:\\Clima\\'+localizacion[0]+'\\Minutales'
    os.chdir(ruta2)
    filenameMin='PSAyMet_'+Anio[i]+'_Min_EOT.xlsx'
    ...
    ClimaTrat=pd.concat([Fecha,ClimaTrat],axis=1)
    rutadia='D:\\Clima\\'+localizacion[0]+'\\Diarios'
    os.chdir(rutadia)
    filename='PSAyMet_'+Anio[i]+'_Dia_EOT.xlsx'
