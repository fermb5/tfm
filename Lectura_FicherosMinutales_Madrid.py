...
for a in range(md):
    if a==0 or ClimaTrat.iloc[dia[a],2]!=ClimaTrat.iloc[dia[a-1],2]: #el primer día o si se cambia de día
        ...
        ruta2='D:\\Clima\\Madrid\\DatosOriginales\\Adquisición\\diarios\\'+str(Anio[1])+'\\'+str(Anio[1])+'_min'
        os.chdir(ruta2)  
        ...
        if nameArfrisol in listaARFRISOLH:
            ...
            columna=cabeceratemp.isin(['Fecha','Hora','Gh','VVext','DVext','Text_Mastil','H_extMastil','Gv','R']) 
            ...
            if temp2.shape[0]==0:
                print(f'El archivo define las variables pero está vacío el {dianame} del {mesname}')
            else:
                ...
                temp6.columns=['Año','Mes','Dia', 'Hora', 'Min','Gh','VVext','DVext','Text_Mastil','H_extMastil','Gv' ] 
        ...
        ruta3='D:\\Clima\\Madrid\\DatosOriginales\\monitorizacionciemat\\'+str(Anio[1])
        os.chdir(ruta3)
        listaMonitorizacionH=os.listdir(ruta3)
        nameMonitorizacion=dianame+'-'+mesname+'-'+str(Anio[1])+'.txt'
        if nameMonitorizacion in listaMonitorizacionH:
            ...
            columna=cabeceratemp.isin(['Hora','Gh','VVext','DVext','Text_Mastil','H_extMastil','Gv']) 
            #identifica las columnas en las que coincide con el nombre de algunas de esas magnitudes 
            Index=np.where(columna==True)[1] #números de las columnas que cumplen que coincidan con las magnitudes previamente mencionadas
            ...
            temp5=pd.DataFrame({'Año':[Anio[1]]*len(temp2.index),'Mes':[mesname]*len(temp2.index),'Dia':[dianame]*len(temp2.index)},index=temp2.index) #genera las fechas en función del nº de datos que se tenga
...
if not climaMonitorizacion.empty: #en caso de que no este vacio el archivo 
    ...
    climaNumMonitorizacion[['Mes','Dia', 'Hora', 'Min','Gh','VVext','DVext','Text_Mastil','H_extMastil','Gv']] = (climaNumMonitorizacion
        [['Mes','Dia', 'Hora', 'Min','Gh','VVext','DVext','Text_Mastil','H_extMastil','Gv']].apply(lambda x: x.str.replace(',', '.')).astype(float)) #cambia el tipo de dato de str a float cuando es str
    ##Validación valores Text,H,VVext,DVext,Gv y Gh
    #Validación Rango Temperatura: -40 a 60ºC
...
os.chdir(ruta)
ClimaTrat.to_excel(localizacion[0]+'_'+str(Anio[1])+'.xlsx',index=False, engine='openpyxl', na_rep='NaN') #se crea el excel
