...
#         'Text_Mastil': [float('nan')] * len(Rango_de_dias),
#         'H_extMastil': [float('nan')] * len(Rango_de_dias),
...
temp7=pd.DataFrame(columns=range(12))
temp8=pd.DataFrame(columns=range(12))
for a in range(md):
    if a==0 or ClimaTrat.iloc[dia[a],2]!=ClimaTrat.iloc[dia[a-1],2]: #el primer día o si se cambia de día
        ...
        ruta2='D:\\Clima\\Madrid\\DatosOriginales\\Adquisición\\diarios\\'+str(Anio[2])+'\\'+str(Anio[2])+'_min'
        os.chdir(ruta2)  
        nameArfrisol='ARFRISOL_SP3_'+str(ClimaTrat.iloc[dia[a],0])+mesname+dianame+'_min.txt' #nombre del archivo tipo ARFRISOL
        ...
        if nameArfrisol in listaARFRISOLH:
            ...
            if cabeceratemp.shape[1]<10:
                ...
                columna=cabeceratemp.isin(['FJ','Fecha','Hora','Gh','VVext','DVext','Text_Mastil','H_extMastil','Gv','R']) 
                ...
                df_divididas =temp['a'].iloc[4:].str.split(expand=True) #divide las columnas desde la 5 fila (donde hay datos)
                ...
                if temp2.shape[0]==0 or temp2.shape[0]==1:
                    ...
                    temp4=pd.Series(temp3.iloc[:,1]).str.split(pat='-',expand=True) #extrae los datos de fecha y los divide por guiones
                    temp4=temp4[list(temp4.columns[::-1])] #cambia el orden al formato Año Mes Día
                    temp5=pd.Series(temp3.iloc[:,2]).str.split(pat=':',expand=True) #extrae los datos de hora y los divide por guiones
                    temp5=temp5.iloc[:,:2] #cambia el orden al formato hora min
                    temp6=pd.concat([temp4,temp5,temp3.iloc[:,3:],temp3.iloc[:,0]],axis=1) #une todos los dataframes
                    ...
                    temp6.columns=['Año','Mes','Dia', 'Hora', 'Min','Gh','VVext','DVext','Text_Mastil','H_extMastil','Gv','FJ' ] 
        ...
        ruta3='D:\\Clima\\Madrid\\DatosOriginales\\monitorizacionciemat\\'+str(Anio[2])
        os.chdir(ruta3)
        ...
        if nameMonitorizacion in listaMonitorizacionH:
            ...
            if cabeceratemp.shape[1]<10:
                ...
                columna=cabeceratemp.isin(['FJ','Hora','Gh','VVext','DVext','Text_Mastil','H_extMastil','Gv']) 
                #identifica las columnas en las que coincide con el nombre de algunas de esas magnitudes 
                Index=np.where(columna==True)[1] #números de las columnas que cumplen que coincidan con las magnitudes previamente mencionadas
                ...
                if temp2.shape[0]==1:
                    ...
                    temp3= temp2.iloc[:,Index] #extrae los datos de las columnas que nos interesa
                    temp4=pd.Series(temp3.iloc[:,1]).str.split(pat=':',expand=True) #extrae los datos de hora y los divide por:
                    ...
                    temp6=pd.concat([temp5,temp4,temp3.iloc[:,2:],temp3.iloc[:,0]],axis=1) #une todos los dataframes
                    if temp6.shape[0]!=1440: #en caso de que el dataframe generado no contenga todos los minutos
                        print(f'Hay un error el {dianame} del {mesname}-m')
                        if temp6.shape[1]==11:
                            temp8.columns=temp6.columns
                            temp8=pd.concat([temp8,temp6]) 
                        else:
...
if not climaARFRISOL.empty: #en caso de que no este vacio el archivo 
    ...
    climaNumARFRISOL.loc[climaNumARFRISOL['Text_Mastil']>60.0 ,'Text_Mastil']=math.nan
    climaNumARFRISOL.loc[climaNumARFRISOL['Text_Mastil']<-40.0,'Text_Mastil']=math.nan
    # Validación Rango humedad relativa: 0 a 100 %
    climaNumARFRISOL.loc[climaNumARFRISOL['H_extMastil']>100.5,'H_extMastil']=math.nan
    climaNumARFRISOL.loc[climaNumARFRISOL['H_extMastil']<-0.5,'H_extMastil']=math.nan
    climaNumARFRISOL.loc[(climaNumARFRISOL['H_extMastil']>100.0) & (climaNumARFRISOL['H_extMastil']<100.5),'H_extMastil']=100.0
    climaNumARFRISOL.loc[(climaNumARFRISOL['H_extMastil']<0.0) & (climaNumARFRISOL['H_extMastil']>-0.5),'H_extMastil']=0.0
...
if not climaMonitorizacion.empty: #en caso de que no este vacio el archivo 
    ...
    climaNumMonitorizacion[['Mes','Dia', 'Hora', 'Min','Gh','VVext','DVext','Text_Mastil','H_extMastil','Gv','FJ']] = (climaNumMonitorizacion
        [['Mes','Dia', 'Hora', 'Min','Gh','VVext','DVext','Text_Mastil','H_extMastil','Gv','FJ']].apply(lambda x: x.str.replace(',', '.')).astype(float)) #cambia el tipo de dato de str a float cuando es str
    ##Validación valores Text,H,VVext,DVext,Gv y Gh
    #Validación Rango Temperatura: -40 a 60ºC
    climaNumMonitorizacion.loc[climaNumMonitorizacion['Text_Mastil']>60.0 ,'Text_Mastil']=math.nan
    climaNumMonitorizacion.loc[climaNumMonitorizacion['Text_Mastil']<-40.0,'Text_Mastil']=math.nan
    # Validación Rango humedad relativa: 0 a 100 %
    climaNumMonitorizacion.loc[climaNumMonitorizacion['H_extMastil']>100.5,'H_extMastil']=math.nan
    climaNumMonitorizacion.loc[climaNumMonitorizacion['H_extMastil']<-0.5,'H_extMastil']=math.nan
    climaNumMonitorizacion.loc[(climaNumMonitorizacion['H_extMastil']>100.0) & (climaNumMonitorizacion['H_extMastil']<100.5),'H_extMastil']=100.0
    climaNumMonitorizacion.loc[(climaNumMonitorizacion['H_extMastil']<0.0) & (climaNumMonitorizacion['H_extMastil']>-0.5),'H_extMastil']=0.0
    # Validación Rango velocidad viento: 0 a 30 %
...
#se realiza el mismo procedimiento pero con la otra fuente de datos
...
climaNumARFRISOL_filtrado=climaNumARFRISOL_filtrado[['Año','Mes','Dia', 'Hora', 'Min','Text_Mastil','H_extMastil','Gh','VVext','DVext','Gv','R']] #se reestructuran las columnas acorde a ClimaTrat
climaNumMonitorizacion_filtrado=climaNumMonitorizacion_filtrado[['Año','Mes','Dia', 'Hora', 'Min','Text_Mastil','H_extMastil','Gh','VVext','DVext','Gv','R']]
...
for index, fila in climaNumMonitorizacion_filtrado.iterrows():
    if index in ClimaTrat.index:
        if ClimaTrat.loc[index,'Text_Mastil':].isnull().all():
            ClimaTrat.loc[index]=fila
...
ruta1='D:\\Clima\\'+localizacion[0]+'\\Minutales'
os.chdir(ruta1)
