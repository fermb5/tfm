...
for i in range(len(Anio)):
    ...
    os.chdir(ruta)
    filenameDiario='MadridyMet_'+Anio[i]+'_Dia.xlsx'
...
for h in range(len(Anio)):
    ...
    filenameDiario2='MadridyMet_'+Anio[0]+'_Dia.xlsx'
    ...
    os.chdir(rutaAnexo2)
    with pd.ExcelWriter('Tabla_Anexo2_'+str(Anio[0])+'.xlsx') as writer:
