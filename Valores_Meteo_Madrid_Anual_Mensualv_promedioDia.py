...
for i in range(len(Anio)):
    ...
    os.chdir(ruta1)
    filenameDia='MadridyMet_'+Anio[i]+'_Dia_Mastil.xlsx'
    ClimaDia=pd.read_excel(filenameDia)
    
    ruta2='D:\\Clima\\'+localizacion[0]+'\\Minutales'
    os.chdir(ruta2)
    filenameMin='MadridyMet_'+Anio[i]+'_Min_EOT.xlsx'
    ClimaMin=pd.read_excel(filenameMin)
    if ClimaMin.shape[1]>18:
        Velocidad=pd.concat([ClimaMin.iloc[:,8],ClimaMin.iloc[:,18]],axis=1)
    else:
        Velocidad=pd.DataFrame(ClimaMin.iloc[:,8])
    Vmax=math.trunc(Velocidad.max().max())+1
    cabecera=['Año','Mes','Dia','TextMastil_prm','TextMastil_max','TextMastil_min',
    'HextMastil_prm','HextMastil_max','HextMastil_min',
    'Gh prm','Gh_max','Gh_min','Gh_sumaHorasMes','Gh_numHoras',
    'VVext_prm','VVext_max','VVext_min',
    'DVextN_prm','VVextN_prm','DVextN_num','DVextNE_prm' ,'VVextNE_prm','DVextNE_num',
    'DVextE_prm','VVextE_prm','DVextE_num','DVextSE_prm','VVextSE_prm','DVextSE_num',
    'DVextS_prm','VVextS_prm','DVextS_num','DVextSO_prm','VVextSO_prm','DVextSO_num',
    'DVextO_prm','VVextO_prm','DVextO_num','DVextNO_prm','VVextNO_prm','DVextNO_num']
    for j in range(Vmax):
        velo='V'+str(Vmax)
        cabecera.append(velo)
    ClimaTrat['AñoMes']=ClimaTrat.apply(lambda row: f"{row['Año']}-{row['Mes']}", axis=1) #agrupa las columnas nombradas con el formato correspondiente
    ClimaTrat.drop(['Año','Mes','Dia','Hora'],axis=1,inplace=True) #elimina las columnas ya agrupadas y la minutal
    ClimaTrat['AñoMes']=pd.to_datetime(ClimaTrat['AñoMes'], format='%Y.0-%m.0') # ordena las columnas en formato horario
    ...
    tempVDVext['AñoMes']=tempVDVext.apply(lambda row: f"{row['Año']}-{row['Mes']}", axis=1) #agrupa las columnas nombradas con el formato correspondiente
    tempVDVext.drop(['Año','Mes','Dia','Hora'],axis=1,inplace=True) #elimina las columnas ya agrupadas y la minutal
    tempVDVext['AñoMes']=pd.to_datetime(tempVDVext['AñoMes'], format='%Y.0-%m.0') # ordena las columnas en formato horario
    tempVDVext = tempVDVext.groupby('AñoMes').apply(lambda group: group.apply(lambda col: eliminar_valores_repetidos(col) if col.name != 'AñoMes' else col))
    tempVDVext = tempVDVext.groupby('AñoMes').apply(lambda group: group.apply(lambda col: filtrar_outliers(col) if col.name != 'AñoMes' else col))
    ...
    VDVNE=VDVNE.groupby('AñoMes').agg({
        'VVext':['mean'],'DVext':['mean','count']})
    ...
    VDVSE=VDVSE.groupby('AñoMes').agg({
        'VVext':['mean'],'DVext':['mean','count']})
    ...
    VDVSO=VDVSO.groupby('AñoMes').agg({
        'VVext':['mean'],'DVext':['mean','count']})
    ...
    VDVNO=VDVNO.groupby('AñoMes').agg({
        'VVext':['mean'],'DVext':['mean','count']})
    ...
    ClimaDia['AñoMes']=ClimaDia.apply(lambda row: f"{row['Año']}-{row['Mes']}", axis=1)
    ClimaDia.drop(['Año','Mes','Dia'],axis=1,inplace=True)
    ClimaDia['AñoMes']=pd.to_datetime(ClimaDia['AñoMes'], format='%Y.0-%m.0')
    ClimaDia=ClimaDia.groupby('AñoMes').agg({
        'Text_prm':['mean'],'Hext_prm':['mean']})
    ClimaDia.columns=[['Text_prm2','Hext_prm2']]
    ClimaDia.reset_index(inplace=True)
    ClimaDia.drop(['AñoMes'],axis=1,inplace=True)
    ClimaTrat=pd.concat([Fecha.iloc[:,:2],ClimaTrat,ClimaDia],axis=1)
    ClimaTrat=ClimaTrat.sort_index()
    ClimaTrat=ClimaTrat[['Año', 'Mes','Text_prm','Text_prm2','Text_max','Text_min','Hext_prm','Hext_prm2','Hext_max','Hext_min',
                        'Gh_prm','Gh_max','Gh_min','Gh_suma','Gh_num','VVextN_prm','DVextN_prm','DVextN_num','VVextNE_prm','DVextNE_prm',
                        'DVextNE_num','VVextE_prm','DVextE_prm','DVextE_num','VVextSE_prm','DVextSE_prm','DVextSE_num','VVextS_prm',
                        'DVextS_prm','DVextS_num','VVextSO_prm','DVextSO_prm','DVextSO_num','VVextO_prm','DVextO_prm','DVextO_num',
                        'VVextNO_prm','DVextNO_prm','DVextNO_num']]
    rutames='D:\\Clima\\'+localizacion[0]+'\\Mensuales'
    os.chdir(rutames)
    filename='MadridyMet_'+Anio[i]+'_Mes_EOT.xlsx'
