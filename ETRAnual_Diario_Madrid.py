...
for i in range(len(Anio)): 
    ...
    TmaxAnual=temp.iloc[:,4].max()
    TminAnual=temp.iloc[:,5].min()
    tempanual=pd.DataFrame({'Año':[Anio[i]],'ETRAnual': [TmaxAnual-TminAnual],'TmaxAnual': [TmaxAnual], 'TminAnual':[TminAnual]})
    ETRAnual=pd.concat([ETRAnual,tempanual],ignore_index=True)
filename='Madrid_ETRAnual_movmedian10.xlsx'
